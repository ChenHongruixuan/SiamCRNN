...
#                         default='/Users/ivanvirovski/Documents/uni/SiamCRNN-1/FCN_version')
...
class Trainer(object):
    ...
    def training(self):
        ...
        best_round = []
        self.deep_model.train()
        class_weight = torch.FloatTensor([1, 10])  # No need to move class_weight to GPU
        elem_num = len(self.train_data_loader)
        train_enumerator = enumerate(self.train_data_loader)
        for _ in tqdm(range(elem_num)):
            itera, data = train_enumerator.__next__()
            ...
            pre_img = pre_img.float()  # Remove .cuda()
            post_img = post_img.float()  # Remove .cuda()
            bcd_labels = bcd_labels.long()  # Remove .cuda()
            # input_data = torch.cat([pre_img, post_img], dim=1)
    ...
    def validation(self):
        ...
        for itera, data in enumerate(val_data_loader):
            ...
            # bcd_output = self.deep_model(input_data)
            bcd_output = self.deep_model(pre_img, post_img)
            bcd_output = bcd_output.data.numpy()  # Remove .cpu()
            bcd_output = np.argmax(bcd_output, axis=1)
            ...
            bcd_labels = bcd_labels.numpy()  # Remove .cpu()
            self.evaluator.add_batch(bcd_labels, bcd_output)
...
def main():
    ...
    parser.add_argument('--data_name_list', type=list)
    parser.add_argument('--start_iter', type=int, default=0)
    parser.add_argument('--cuda', type=bool, default=False)  # Set CUDA to False
    parser.add_argument('--crop_size', type=int, default=256)
    parser.add_argument('--max_iters', type=int, default=100000)
    parser.add_argument('--model_type', type=str, default='SiamCRNN')
    parser.add_argument('--model_param_path', type=str, default='../saved_models')
